...
const Average = () => {
  ...
  const elements = [];
  const week_one_closure = (
    Subtraction(firstWeekClosed) /
    (firstWeekOpen[0] + firstWeekOpen[1])
  ).toFixed(2)

  const week_two_closure = (
    Subtraction(secondWeekClosed) /
    (secondWeekOpen[0] + secondWeekOpen[1])
  ).toFixed(2);
  const week_three_closure = (
    Subtraction(thirdWeekClosed) /
    (thirdWeekOpen[0] + thirdWeekOpen[1])
  ).toFixed(2);
  const week_four_closure = (
    Subtraction(fourthWeekClosed) /
    (fourthWeekOpen[0] + fourthWeekOpen[1])
  ).toFixed(2);
  const week_five_closure = (
    Subtraction(fifthWeekClosed) /
    (fifthWeekOpen[0] + fifthWeekOpen[1])
  ).toFixed(2);
  const week_six_closure = (
    Subtraction(sixthWeekClosed) /
    (sixthWeekOpen[0] + sixthWeekOpen[1])
  ).toFixed(2);
  const week_seven_closure = (
    Subtraction(seventhWeekClosed) /
    (seventhWeekOpen[0] + seventhWeekOpen[1])
  ).toFixed(2);
  const week_eight_closure = (
    Subtraction(eighthWeekClosed) /
    (eighthWeekOpen[0] + eighthWeekOpen[1])
  ).toFixed(2);
  const week_nine_closure = (
    Subtraction(ninthWeekClosed) /
    (ninthWeekOpen[0] + ninthWeekOpen[1])
  ).toFixed(2);
  const week_ten_closure = (
    Subtraction(tenthWeekClosed) /
    (tenthWeekOpen[0] + tenthWeekOpen[1])
  ).toFixed(2);

  elements.push(
    Number(week_one_closure),
    Number(week_two_closure),
    Number(week_three_closure),
    Number(week_four_closure),
    Number(week_five_closure),
    Number(week_six_closure),
    Number(week_seven_closure),
    Number(week_eight_closure),
    Number(week_nine_closure),
    Number(week_ten_closure)
  );

  const sum = elements.reduce((partialSum, a) => partialSum + a, 0);
  ...
  return (
    <>
    {isLoading ? 
      <h2>Average Weekly Closure
      <div> {(sum/elements.length).toFixed(2)}</div>
      </h2>
  : (
